...
import (
	...
	"github.com/josecar25/gorgonia/internal/encoding"
	"gorgonia.org/tensor"
)
...
func Conv2d(im, filter *Node, kernelShape tensor.Shape, pad, stride, dilation []int) (retVal *Node, err error) {
        ...
        if im.Shape().Dims() == 3 {
                im, _ = Reshape(im, tensor.Shape{im.Shape()[0], im.Shape()[1], 1, im.Shape()[2]})
                //return nil, fmt.Errorf("im should have 4 dims, got %v dims", im.Shape().Dims())
        }

        if filter.Shape().Dims() == 3 {
                filter, _ = Reshape(filter, tensor.Shape{filter.Shape()[0], filter.Shape()[2], 1, filter.Shape()[1]})
//              return nil, fmt.Errorf("filter should have 4 dims, got %v dims", filter.Shape().Dims())
        ...
        var colIm *Node
        fmt.Println("JOSE5")
        fmt.Println(kernelShape)
        kernelShape = tensor.Shape{1,kernelShape[0]}
        fmt.Println(kernelShape)
        fmt.Println(dilation)
        dilation = []int{1, dilation[0]}
        fmt.Println(dilation)
        fmt.Println(filter)
        pad = []int{0, pad[0]}
        fmt.Println("TSTSTSTSTSTS")
        fmt.Println(pad)
